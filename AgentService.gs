...
function getUserRole(ldap) {
  ...
  var cacheKey = 'user_role_' + ldap.toLowerCase();
  var cached = getCached(cacheKey);
  if (cached) return cached;

  // Check Managers sheet (contains both Managers and Supervisors)
  var allManagers = getSheetDataCached('Managers', 3600);
  var managerRow = allManagers.find(function(m) { return m['LDAP'] === ldap; });
  if (managerRow) {
    var role = String(managerRow['Role']).toLowerCase();
    var finalRole = (role === 'manager' || role === 'supervisor') ? role : 'manager';
    setCached(cacheKey, finalRole, 1800); // 30 min
    return finalRole;
  }

  // Check Agents sheet
  var allAgents = getSheetDataCached('Agents', 600);
  var agent = allAgents.find(function(a) { return a['LDAP'] === ldap; });
  if (agent) {
    setCached(cacheKey, 'agent', 1800);
    return 'agent';
  }
...
// Deprecated: Tier recalculation is now handled inside addPoints to batch operations.
function recalculateTier(ldap) {
  addPoints(ldap, 0, 'Recalculating tier');
}
...
function addPoints(ldap, points, reason) {
  ...
  var tier = getTierForPoints(newAllTime);
